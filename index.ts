...
import { Person, Gender } from './assets/person';
...
// Running variables
let offspring: number = 0; // setChildPerGeneration()
...
function getDonor(parent_1: Person): Person | boolean {
  for (let i = humans.length - 1; i >= 0; i--) {
    ...
    if (donor.getDonorEligible(parent_1)) {
      return donor;
    }
  }
  return false;
...
function randomEvent(): boolean {
  if (CONFIG.random_event_chance === 0) {
    return false;
  }
  return (Math.random() <= CONFIG.random_event_chance) ? true : false;
}

function mate(parent_1?: Person, parent_2?: Person): boolean {
  function newHuman(parent_1: Person, parent_2: Person) {
    let new_human = new Person(parent_1, parent_2, undefined, undefined, undefined, generation, undefined);
    offspring_counter += 1;
  ...
  if (parent_1.getHomosexualStatus() == true || parent_2.getHomosexualStatus() == true) {
    ...
    if (randomEvent() == true) {
      ...
      console.log({random_event_offspring_counter});
  ...
  // Non homosexual couple
  if (parent_1.getHomosexualStatus() == false && parent_2.getHomosexualStatus() == false) {
    newHuman(parent_1, parent_2);
    return true;
  }
  return false;
...
function checkMate(parent_1?: Person, parent_2?: Person): boolean {
  // Maximum breeding age is lower than death range
  if (!parent_1 || !parent_2) {
    return false;
  }
  if (parent_1 == parent_2) {
...
function initPopulation(): void {
  ...
  for (let i = 0; i < (CONFIG.starting_population * CONFIG.starting_lgbt_percent); i++) {
    let gender = ((humans.length % 2) == 0) ? Gender.M : Gender.F;
    let donor_random: boolean = (Math.random() <= 0.5);
    let age: number = Math.floor(Math.random() * (50 - 1) + 1);
    let new_human: Person = new Person(undefined, undefined, age, true, donor_random, undefined, gender);
    humans.push(new_human);
  }
  ...
  while (humans.length < CONFIG.starting_population) {
    ...
    let donor_random: boolean = (Math.random() <= 0.5);
    let age: number = Math.floor(Math.random() * (50 - 1) + 1);
    let new_human = new Person(undefined, undefined, age, false, donor_random, undefined, gender);
    humans.push(new_human);
  }
}
...
function main(): void {
  ...
  while (checkExtinct(false) == false || run_off_counter < CONFIG.run_off) {
    ...
    if (CONFIG.force_homosexual_random_event_during_generation == true) {
